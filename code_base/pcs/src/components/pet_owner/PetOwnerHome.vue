    ...
    await axios
      ...
      .then((response) => {
        ...
        for (i = 0; i < response.data.length; i++) { 
          ...
          //let job_start_date_string = response.data[i].job_start_datetime.toString().split("T")[0];
          //let job_start_date = new Date(date_string);
          let job_start_date = new Date.parse(response.data[i].job_start_datetime.toString());
          //console.log(job_start_date)
          if (job_start_date >= myToday && job_start_date < myTomorrow){
            comment_arr_temp.push("Start day")
          }

          let job_end_date = new Date.parse(response.data[i].job_end_datetime.toString());
          //console.log(job_start_date)
